...
- **TLS cert SANs** — the leaf cert (signed by the device CA — see
  [docs/CERTS.md](docs/CERTS.md)) covers every entry (DNS and IP)
...
has its own authentication) chains to the same trust anchor as `:443`
...
The device runs a small local CA that signs a single leaf used by Traefik
...
├── docs/
│   ├── CERTS.md            # Local CA, leaf, cockpit sharing, /halos-ca.crt
│   ├── HOSTNAMES.md        # hostnames.conf reference
